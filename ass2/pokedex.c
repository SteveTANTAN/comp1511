...
// the fuction iterate the whole list to find out 
// the pokemon with the id given.
static struct pokenode *search_id (Pokedex pokedex, int id) {
    struct pokenode *node = pokedex->head;
...
// the fuction is used to find out the current_select pokemon 
...
// the fuction is used to copy all the types into a array,
// if the type is noy NONE_TYPE.
// before adding, we check if the type has been added before,
// if it has, we do nothing
// if has not, we add the pokemon at the end of list.
static void copy_type_into_string(int *array, struct pokenode *node) {
    int type1 = pokemon_first_type(node->pokemon);
    int temp = 0; 
    int test = TRUE;
    
    // check if the pokemon type has been added before.
    while (array[temp] != '\0') {
        if (type1 == array[temp]) {
            test = FALSE;
        }
...
// the fuction is used to print out all the types which are stored 
// in the type array.
static void print_Out_TotalTYPE (int *array) {
    int temp = 0;
    while (array[temp] != '\0') {
...
// the fuction is used to add types of the pokemon into array 
// and check if it has been added before,
// if yes, we do noting,
// if have not added, we add the new type into array,
// when the next pokedex doing this fuction
// repeat this checking process again.
static void test_first_type (int *array, struct pokenode *node){
    ...
    // check if the type has been added before.
    while (array[temp] != '\0') {
        if (type1 == array[temp]) {
            test = FALSE;
        }
    ...
    //  if has no, we add the type at the end of list,
    if (test == TRUE) {
        node->foundstate = TRUE;
        array[temp] = type1;
    }
...
////////////////          stage  4      //////////////////////////
// the fuction is used to print out the pokemon evolution information.
static void printEVO (struct pokenode *pokenode) {
    printf ("#%03d ", pokemon_id(pokenode->pokemon));
    if (pokenode->foundstate == TRUE) {
...
// the fuction is used to add the pokenode into the new pokedex.
// and set all the added pokemons be found.
// if there is no pokenode in the new pokedex, 
// adding the pokemon at first, 
// and set the first pokemon to be the current_select pokemon
static void add_in_new_pokedex (Pokedex pokedex, struct pokenode *node) {
    struct pokenode *new = pokedex->head;
    if (new == NULL) {
...
// the fucton is used to check if the pokemon 
// has the same type as the given type. 
// if yes, return TRUE.
static int compare(struct pokenode *node, pokemon_type type) {
    int count = FALSE;
...
// this is used for sort the pokemon list by pokemonID.
static void order_pokenode(Pokedex pokedex, struct pokenode *node) {
...
// the fuction used to check if the name of pokemon 
// is similary to the given text. 
static int searchAndCompare (char *text, struct pokenode *node) {
    char *new_text = text;
    ...
    int temp = 0;
    int count = 0;
    // iterate the whole loop, if the name has one letter 
    // is as same as the first letter of given test,
    // then, we check if the rest letter in given text 
    // is as same as the pokemon name by using [if_text_in_name]
    // if [if_text_in_name] fail, we check for the next same letter
    // until the iterate over. 
...
// the fuction is used to check if the rest letter in given text 
// is as same as the pokemon name. 
// if all the letter same, return TRUE
// if not, return FALSE.
