...
void pad_left(struct character_node *characters, char pad_character) {
    struct character_node *node = characters;
    struct character_node *newnode;
    newnode->data = pad_character;
    newnode->next = node;
    struct character_node *nodeend = newnode;
    while (nodeend->next->next != NULL) {
        nodeend = nodeend->next;
    }
    struct character_node *temp = nodeend->next;
    nodeend->next = NULL;
    free(temp);
    return characters;
}
